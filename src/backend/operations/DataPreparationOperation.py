...
class DataPreparationOperation(ParallelizableTextOperation):
    ...
    def single_cell_operation(self, notifier: FrontendNotifier, payload: Payload, text: str) -> str:
        ...
        if self.do_stopwords:
            ...
            self.stats["stop_words_removed"] += len(doc) - len(filtered_tokens)

        # Convert text to lowercase
        if self.do_lowercase:
            original_upper = sum(1 for c1, c2 in zip(text, text.lower()) if c1.isupper() and c2.islower())
            text = text.lower()
            self.stats["text_lowercased"] += original_upper
        ...
        if self.do_no_ascii:
            ...
            text = cleaned_text
            self.stats["non_ascii_removed"] += removed
        ...
        <div style="font-family: Arial, sans-serif; padding: 10px;">
            ...
            <ul style="list-style-type: none; padding: 0;">
                <li><strong>Stop Words Removed:</strong> {self.stats['stop_words_removed']}</li>
                <li><strong>Text Lowercased:</strong> {self.stats['text_lowercased']}</li>
                <li><strong>Non-ASCII Characters Removed:</strong> {self.stats['non_ascii_removed']}</li>
            </ul>
